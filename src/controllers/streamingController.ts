...
class StreamingAnime {
    async create(req: Request, res: Response, next: NextFunction): Promise<void> {
        try {
            ...
            if (parseInt(episode) > animeInfo.numberOfEpisodes) {
                return next(new ApiError('Provided episode does not exist', 404));
            };


            const path = new StreamingService().createURl(animeInfo.category, animeInfo.name.toLowerCase().replace(/\s/g, '_'), episode);
